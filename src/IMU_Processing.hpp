...
// eigen_conversions/eigen_msg.h removed for ROS2 port; add tf2_eigen when needed
...
{
  RCLCPP_WARN(rclcpp::get_logger("pointlio_mapping"), "Reset ImuProcess");
  mean_acc      = V3D(0, 0, -1.0);
  mean_gyr      = V3D(0, 0, 0);
  imu_need_init_    = true;
  init_iter_num     = 1;
...
void ImuProcess::IMU_init(const MeasureGroup &meas, int &N)
{
  ...
  RCLCPP_INFO(rclcpp::get_logger("pointlio_mapping"), "IMU Initializing: %.1f %%", double(N) / MAX_INI_COUNT * 100);
  V3D cur_acc, cur_gyr;
...
{  
  ...
  assert(meas.lidar != nullptr);
    ...
    if (imu_need_init_)
    {
      ...
      {
  RCLCPP_INFO(rclcpp::get_logger("pointlio_mapping"), "IMU Initializing: %.1f %%", 100.0);
