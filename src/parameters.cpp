...
void readParameters(rclcpp::Node::SharedPtr node)
{
  ...
  vel_cov = node->declare_parameter<double>("mapping.vel_cov", 20.0);
  ...
  match_s = node->declare_parameter<double>("mapping.match_s", 81.0);
  ...
  publish_cloud_qos = node->declare_parameter<std::string>("publish.cloud_qos", "reliable");
}
